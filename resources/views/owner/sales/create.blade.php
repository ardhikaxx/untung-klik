...
@section('title', 'Kasir & Catat Penjualan')

@section('content')
<!-- Page Header -->
<div class="uk-page-header">
    <div>
        <h2 class="uk-page-title">Kasir & Catat Penjualan</h2>
        <p class="uk-page-subtitle">Pilih produk secara cepat atau pilih dari keranjang. Stok otomatis terpotong dan tercatat di Buku Kas.</p>
    </div>
    <div class="d-flex align-items-center gap-2 flex-wrap">
        <a href="{{ route('owner.sales.index') }}" class="btn btn-sm btn-uk-outline rounded-pill px-3">
            <i class="fas fa-history me-1.5"></i>Riwayat Penjualan
        </a>
        <a href="{{ route('owner.products.create') }}" class="btn btn-sm btn-uk-secondary rounded-pill px-3">
...
@if($products->isEmpty())
    <div class="uk-card text-center p-4 p-md-5">
        <div class="uk-empty-icon">
            <i class="fas fa-box-open"></i>
        </div>
        <h5 class="fw-bold text-dark mb-1">Belum Ada Katalog Produk</h5>
        <p class="text-muted small mb-3">Anda perlu menambahkan produk terlebih dahulu sebelum mencatat penjualan kasir.</p>
        <a href="{{ route('owner.products.create') }}" class="btn btn-sm btn-uk-primary rounded-pill px-4">
            <i class="fas fa-plus me-1"></i>Tambah Produk Sekarang
        </a>
    </div>
@else
<form method="POST" action="{{ route('owner.sales.store') }}" id="saleForm">
    @csrf
    ...
    <div class="row g-4">
                    ...
                    <div class="row g-2 align-items-center">
                        <div class="col-md-6">
                            <h6 class="fw-bold mb-0 text-dark">
                                <i class="fas fa-th-large text-primary me-2"></i>Pilih Cepat Produk (Klik untuk Memasukkan)
                            </h6>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="row g-2 overflow-auto" id="quickProductsGrid" style="max-height: 280px;">
                        @foreach($products as $p)
                            <div class="col-6 col-sm-4 col-md-3 quick-product-card"
                                 ...
                                 data-unit="{{ $p->unit }}">
                                <div class="card h-100 border text-start p-3 {{ $p->stock <= 0 ? 'bg-light opacity-50' : 'cursor-pointer hover-shadow product-card-btn' }}"
                                     onclick="{{ $p->stock > 0 ? "addFromQuickGrid({$p->id})" : '' }}"
                                     style="border-radius: var(--uk-radius-sm); border-color: var(--uk-border); transition: all 0.15s ease-in-out; {{ $p->stock > 0 ? 'cursor: pointer;' : 'cursor: not-allowed;' }}">
                                    <div class="d-flex justify-content-between align-items-start mb-1">
                                        <span class="badge {{ $p->stock <= 0 ? 'bg-danger' : ($p->stock <= $p->min_stock ? 'bg-warning text-dark' : 'bg-light text-dark border') }}" style="font-size: 0.65rem;">
            ...
            <div class="uk-card p-0 overflow-hidden">
                ...
                <div class="p-0">
                    <div class="table-responsive mb-0">
                        <table class="table uk-table align-middle mb-0" id="itemsTable">
                            <thead>
                                <tr>
                                    <th class="ps-4" style="min-width: 230px;">Produk</th>
                                    <th style="width: 130px;" class="text-end">Harga Satuan</th>
                                    <th style="width: 140px;" class="text-center">Jumlah</th>
                                    <th style="width: 140px;" class="text-end">Subtotal</th>
                                    <th style="width: 50px;" class="text-center pe-4"></th>
                                </tr>
                            </thead>
                    ...
                    <!-- Placeholder bila keranjang kosong -->
                    <div id="emptyCartNotice" class="uk-empty-state py-5 text-center">
                        <div class="uk-empty-icon">
                            <i class="fas fa-cart-arrow-down"></i>
                        </div>
                        <div class="uk-empty-title">Keranjang Penjualan Masih Kosong</div>
                        <p class="uk-empty-desc mb-0">Klik produk pada kotak di atas atau klik tombol "+ Tambah Baris" untuk memilih barang.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-xl-4 col-lg-5">
            <div class="uk-card p-0 overflow-hidden uk-sticky-card">
                <div class="p-3 px-4 border-bottom bg-white d-flex align-items-center justify-content-between">
                    <h6 class="fw-bold mb-0 text-dark">
                        <i class="fas fa-cash-register text-primary me-2"></i>Pembayaran & Kasir
                    </h6>
                    <span class="badge bg-light text-dark border">Kasir Toko</span>
                </div>
                ...
                <!-- Info Pelanggan -->
                <div class="mb-3">
                    <label for="customer_name" class="form-label small fw-semibold text-dark mb-1">Nama Pelanggan (Opsional)</label>
                    <div class="input-group input-group-sm">
                        <span class="input-group-text bg-light"><i class="fas fa-user text-muted"></i></span>
                        <input type="text" class="form-control" id="customer_name" name="customer_name"
                               placeholder="Contoh: Pelanggan Umum" value="{{ old('customer_name', 'Pelanggan Umum') }}">
                    </div>
                </div>

                <div class="mb-3">
                    <label for="customer_phone" class="form-label small fw-semibold text-dark mb-1">No. WhatsApp / Telepon (Opsional)</label>
                    <div class="input-group input-group-sm">
                        <span class="input-group-text bg-light"><i class="fab fa-whatsapp text-muted"></i></span>
                        <input type="text" class="form-control" id="customer_phone" name="customer_phone"
                               placeholder="0812xxxx (untuk nota WA)" value="{{ old('customer_phone') }}">
                    </div>
                </div>

                <div class="row g-2 mb-3">
                    <div class="col-sm-6">
                        <label for="transaction_date" class="form-label small fw-semibold text-dark mb-1">Tanggal <span class="text-danger">*</span></label>
                        <input type="date" class="form-control form-control-sm" id="transaction_date" name="transaction_date"
                               value="{{ old('transaction_date', date('Y-m-d')) }}" required>
                    </div>
                    <div class="col-sm-6">
                        <label for="payment_method" class="form-label small fw-semibold text-dark mb-1">Metode Bayar <span class="text-danger">*</span></label>
                ...
                <!-- Grand Total Banner (Clean styled with brand primary) -->
                <div class="p-3 rounded-3 mb-3 text-center" style="background-color: rgba(51, 153, 137, 0.08); border: 1px solid rgba(51, 153, 137, 0.25);">
                    ...
                    <div class="fs-3 fw-bold" style="color: var(--uk-primary); letter-spacing: -0.02em;" id="grandTotalDisplay">Rp 0</div>
                    <span class="text-muted small" id="itemsCountBadge">0 jenis (0 unit)</span>
                </div>

                <!-- Input Pembayaran Tunai & Kembalian -->
                <div id="cashCalculationArea">
                    <div class="mb-2">
                        <label for="cash_received" class="form-label small fw-semibold text-dark mb-1">Uang Diterima Pelanggan (Rp)</label>
                        <div class="input-group input-group-sm">
                            <span class="input-group-text bg-light">Rp</span>
                ...
                <div class="mb-3">
                    <label for="description" class="form-label small fw-semibold text-dark mb-1">Catatan Struk (Opsional)</label>
                ...
                <a href="{{ route('owner.sales.index') }}" class="btn btn-uk-secondary w-100 mt-2 btn-sm rounded-pill">Batal</a>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    // Initialize with 1 empty row or first product if available
    ...
    function filterByCategory(catId, btn) {
        document.querySelectorAll('.category-filter-btn').forEach(b => {
            b.classList.remove('btn-uk-primary', 'active');
            b.classList.add('btn-uk-ghost');
        });
        btn.classList.remove('btn-uk-ghost');
        btn.classList.add('btn-uk-primary', 'active');
            ...
            cart.forEach((item, index) => {
                ...
                tr.innerHTML = `
                    ...
                    <td class="text-center pe-4">
                        <button type="button" class="btn btn-xs btn-uk-danger rounded-pill px-2" onclick="removeCartItem(${index})" title="Hapus Item">
